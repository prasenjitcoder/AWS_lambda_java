...
def lambda_handler(event, context):
    try:
        ...
        ##Do some chekcing to validate the authToken
        if(authToken=="1234567"):
            success = "true"
            logger.info('Authtoken Validated')
        else:
            success = "false"

        policy = buildAPIDocumentPolicy(success,event,authToken)
        # initiate the authorizer context
        context = buildContextAuthorize(authToken)
        # Finally, build the policy
        authResponse = policy.build()
        # Add to the  JSON  context info for context if needed !.
...
def buildContextAuthorize(authToken):
    context = {}
    context['authToken'] = authToken
    return context


def extractApiDetails(event):
    try:
        methodArn = event['methodArn']
        tmp = methodArn.split(':')
        apiGatewayArnTmp = tmp[5].split('/')
        apiId = apiGatewayArnTmp[0]
        apiStage = apiGatewayArnTmp[1]
        return apiId, apiStage
    except Exception as e:
        logger.error(e)


def buildAPIDocumentPolicy(success, event,authToken):
    apiId, stage = extractApiDetails(event)
    policy = AuthPolicy(authToken, "yyyyy")
    policy.restApiId = apiId
    policy.region = "us-east-6"
    policy.stage = stage
    if(success=="true"):
        policy.allowMethod(HttpVerb.GET, '/hello/sayhello')
    else:
        policy.denyAllMethods()
    return policy
...
class Unauthorized(Exception):
    "Handles any authentication failure or internal authorizer exception"
    pass

class ExpiredToken(Exception):
    "Handles expiration token exception"
    pass
